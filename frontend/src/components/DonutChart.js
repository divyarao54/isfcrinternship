...
const DonutChart = ({ donutData, onSegmentClick }) => {
    ...
    const hIndexData = donutData.teachers.map((t, idx) => ({
        name: t.name,
        value: t.hIndex,
        layer: 'hIndex',
        fill: HINDEX_COLORS[idx % HINDEX_COLORS.length],
    }));
    const i10IndexData = donutData.teachers.map((t, idx) => ({
        name: t.name,
        value: t.i10Index,
        layer: 'i10Index',
        fill: I10_COLORS[idx % I10_COLORS.length],
    }));
    const publicationsData = donutData.teachers.map((t, idx) => ({
        name: t.name,
        value: t.publications,
        layer: 'publications',
        fill: PUBS_COLORS[idx % PUBS_COLORS.length],
    }));
